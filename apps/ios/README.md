...
This is an **alpha** iOS app that connects to an CDog Gateway as a `role: node`.
...
3. On iOS: CDog → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project CDog.xcodeproj -scheme CDog -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/CDogKit` contains the shared transport/types used by the iOS app.
