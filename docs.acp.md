...
This document describes how the CDog ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`cdog acp` exposes an ACP agent over stdio and forwards prompts to a running
CDog Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a CDog Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `cdog acp` over stdio.

Example config:

```bash
cdog config set gateway.remote.url wss://gateway-host:18789
cdog config set gateway.remote.token <token>
```

Example run:

```bash
cdog acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
cdog acp --session agent:main:main
cdog acp --session agent:design:main
cdog acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “CDog ACP” to start a thread.

## Execution Model

- ACP client spawns `cdog acp` and speaks ACP messages over stdio.
...
```bash
cdog acp --session agent:main:main
cdog acp --session-label "support inbox"
cdog acp --reset-session
```
...
`cdog acp` resolves the Gateway URL and auth from CLI flags or config:
