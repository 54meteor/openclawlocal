---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for CDog deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, CDog cron scheduling for periodic checks, or version status checks on a machine running CDog (laptop, workstation, Pi, VPS).
---
...
Assess and harden the host running CDog, then align it to a user-defined risk tolerance without breaking access. Use CDog security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim CDog changes the host firewall, SSH, or OS updates; it does not.
...
5. CDog gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if CDog can access sensitive data.
...
### 2) Run CDog security audits (read-only)

As part of the default read-only checks, run `cdog security audit --deep`. Only offer alternatives if the user requests them:

1. `cdog security audit` (faster, non-probing)
2. `cdog security audit --json` (structured output)

Offer to apply CDog safe defaults (numbered):

1. `cdog security audit --fix`

Be explicit that `--fix` only tightens CDog defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check CDog version/update status (read-only)

As part of the default read-only checks, run `cdog update status`.
...
- Credential hygiene notes (location of CDog creds, prefer disk encryption)
...
After CDog install or first hardening pass, run at least one baseline audit and version check:

- `cdog security audit`
- `cdog security audit --deep`
- `cdog update status`

Ongoing monitoring is recommended. Use the CDog cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `cdog cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `cdog update status`
...
Before creating, `cdog cron list` and match on exact `name`. If found, `cdog cron edit <id> ...`.
If not found, `cdog cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `cdog update status` (preferred for source checkouts and channels)
2. `npm view cdog version` (published npm version)

## CDog command accuracy

Use only supported commands and flags:

- `cdog security audit [--deep] [--fix] [--json]`
- `cdog status` / `cdog status --deep`
- `cdog health --json`
- `cdog update status`
- `cdog cron add|list|runs|run`

Do not invent CLI flags or imply CDog enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by CDog compaction:
