...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "cdog.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "openclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "cdog.js", entryBasename: "entry.js" },
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.CDOG_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.CDOG_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.CDOG_NODE_OPTIONS_READY = "1";
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[cdog] ${parsed.error}`);
      process.exit(2);
    }
